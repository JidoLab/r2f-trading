/**
 * Weekly newsletter sender — shared logic used by both the cron endpoint
 * and the admin "Send Now" button. Avoids HTTP round-trips between
 * /api/admin/newsletters/send and /api/cron/send-newsletter.
 */
...
export async function sendWeeklyNewsletter(): Promise<NewsletterSendResult> {
  // 1. Get subscribers
  ...
  try {
    ...
    return { sent: 0, failed: 0, total: 0, subject: "", articles: 0, video: false, message: "No subscribers" };
  }
  ...
  const recentShorts: { title: string; youtubeUrl?: string; slug: string; createdAt: string }[] = [];
  try {
...
- ctaType: alternate between "call" (book discovery call) and "kit" (check out starter kit)`;
  ...
  // Save newsletter copy to GitHub
